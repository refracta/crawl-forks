...
void make_book_legion_gift(item_def &book, bool first)
{
    ...
    if (first)
    {
        ...
        chosen_spells[0] = SPELL_SUMMON_SMALL_MAMMAL;
        chosen_spells[1] = SPELL_CALL_CANINE_FAMILIAR;
        chosen_spells[2] = SPELL_RECALL;
		chosen_spells[3] = coinflip() ? SPELL_SUMMON_BUTTERFLIES : SPELL_CALL_IMP;
        chosen_spells[4] = coinflip() ? SPELL_SUMMON_ICE_BEAST : SPELL_SUMMON_LIGHTNING_SPIRE;
    }
    else
    {
        ...
        chosen_spells[2] = coinflip() ? SPELL_HAUNT : SPELL_SUMMON_HYDRA;
        chosen_spells[3] = SPELL_SHADOW_CREATURES;
    }
